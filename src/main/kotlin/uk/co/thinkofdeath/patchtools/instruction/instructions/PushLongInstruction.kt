...
public class PushLongInstruction : InstructionHandler {
    override fun check(classSet: ClassSet, scope: PatchScope?, instruction: PatchInstruction, method: MethodNode, insn: AbstractInsnNode): Boolean {
        ...
        if (instruction.params[0] == "*") {
            ...
            `val` = java.lang.Long.parseLong(instruction.params[0])
        }

        if (insn is LdcInsnNode) {
            if (insn.cst is Long) {
                return !(!any && insn.cst as Long != `val`)
            }
        } else if (insn is InsnNode) {
            if (insn.getOpcode() >= Opcodes.LCONST_0 && insn.getOpcode() <= Opcodes.LCONST_1) {
                val other = (insn.getOpcode() - Opcodes.LCONST_0).toLong()
    ...
    override fun create(classSet: ClassSet, scope: PatchScope, instruction: PatchInstruction, method: MethodNode): AbstractInsnNode {
        ...
        val `val` = java.lang.Long.parseLong(instruction.params[0])
        if (`val` >= 0 && `val` <= 1) {
            return InsnNode((Opcodes.LCONST_0.toLong() + `val`).toInt())
        }
    ...
    override fun print(instruction: Instruction, patch: StringBuilder, method: MethodNode, insn: AbstractInsnNode): Boolean {
        if (insn is LdcInsnNode) {
            if (insn.cst is Long) {
                patch.append("push-long ").append(insn.cst as Long)
                return true
            }
        } else if (insn is InsnNode) {
            if (insn.getOpcode() >= Opcodes.LCONST_0 && insn.getOpcode() <= Opcodes.LCONST_1) {
                patch.append("push-long ").append(insn.getOpcode() - Opcodes.LCONST_0)
                return true
            }
        }
        return false
    ...
    override fun validate(instruction: PatchInstruction) {
        if (instruction.params.size != 1) {
            throw ValidateException("Incorrect number of arguments for push-long")
        }

        try {
            if (instruction.params[0] != "*") {
                java.lang.Long.parseLong(instruction.params[0])
            }
